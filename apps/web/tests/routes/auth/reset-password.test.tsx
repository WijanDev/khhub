...
import { resetPasswordRoute as Route } from '@/features/auth/presentation/routing';
...
describe('Route', () => {
  ...
  it('should have correct route id', () => {
    expect(route.id).toBe('/reset-password');
  });
...
describe('ResetPasswordPage', () => {
  ...
  it('should call resetPassword with correct params when onSubmit is called', async () => {
    ...
    if (onSubmit) {
      await act(async () => {
        await onSubmit({ value: { newPassword: 'newpass123', confirmPassword: 'newpass123' } });
      });
  ...
  it('should set serverError when token is missing in onSubmit', async () => {
    ...
    if (onSubmit) {
      await act(async () => {
        await onSubmit({ value: { newPassword: TEST_NEW_PASSWORD, confirmPassword: TEST_NEW_PASSWORD } });
      });
