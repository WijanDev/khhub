...
describe('Route', () => {
  ...
  it('should have correct route id', () => {
    expect(route.id).toBe('/app/tenants');
  });

  it('should call prefetchTenants in loader when queryClient exists', async () => {
    (globalThis as any).__mockGetQueryClientFromContext__.mockReturnValue(mockQueryClient);
    
    await route.loader({ context: {} });
    
    expect((globalThis as any).__mockGetQueryClientFromContext__).toHaveBeenCalledWith({});
    expect((globalThis as any).__mockPrefetchTenants__).toHaveBeenCalledWith(mockQueryClient);
  });

  it('should not call prefetchTenants when queryClient is null', async () => {
    (globalThis as any).__mockGetQueryClientFromContext__.mockReturnValue(null);
    
    await route.loader({ context: {} });
    
    expect((globalThis as any).__mockGetQueryClientFromContext__).toHaveBeenCalledWith({});
    expect((globalThis as any).__mockPrefetchTenants__).not.toHaveBeenCalled();
  });
});
