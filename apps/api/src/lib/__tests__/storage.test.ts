...
function createMockR2Bucket(): R2Bucket {
  ...
  return {
    put: vi.fn(async (key: string, data: ArrayBuffer | Blob | ReadableStream | string, options?: R2PutOptions) => {
      ...
      const httpMetadata = options?.httpMetadata && typeof options.httpMetadata === 'object' && !('get' in options.httpMetadata)
        ? options.httpMetadata as R2HTTPMetadata
        : undefined;
